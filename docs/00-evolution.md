...
- (발표에서는 이 단계 슬라이드 제외 — 2026-08 결정. 역사 맥락 자료로만 유지)
...
- **현재 모습 (2026.08 조사, 발표 전 헬프센터 재확인)**: 구성 4요소 — 커스텀 지침
  (약 8천 자)·지식 베이스·대화 모음·프로젝트별 분리 메모리(자동 요약). 지식이
  컨텍스트 창을 넘으면 **RAG 검색 모드**로 자동 전환(유료 플랜). 제공 범위:
  Free 5개(2026.02~), Pro·Max·Team·Enterprise 무제한. 팀 공유는 Team·Enterprise
  (역할 기반: 비공개/보기/편집, 지침·지식 공동 관리).
...
- **의미**: 성숙한 제품이라기보다 "모델이 스스로 루프를 도는" 에이전트 방향을
  보여준 실험. (발표에서는 이 단계 슬라이드 제외 — 2026-08 결정. 수렴 장에서
  구두 언급만 가능)
...
| 시기 | 방식 | 무엇이 달라졌나 |
|------|------|----------------|
| 2023 초기 | 매 대화 프롬프트에 직접 쓰기 (**페르소나/역할 부여** 등) | "너는 ~전문가야" — 역할·규칙·맥락을 매번 반복 입력 |
| 2023.11 | **시스템 프롬프트** (Claude 2.1) | 지시를 대화에서 분리해 고정 (개발자용 API 기능) |
| (도입 시기 비공식) | **개인 지침** — claude.ai 프로필 "Instructions for Claude" | 계정 **전역**에 적용되는 사용자 수준 지시 (Cowork 포함 모든 대화) |
| 2024.06 | **Projects** 커스텀 지침 + 지식 | 일반 사용자도 지시·지식을 담아 재사용 (**프로젝트 단위**) |
...
2. **개인 → 팀 → 생태계.** 프롬프트(개인) → Projects(팀 공유) →
   ...
   단위가 커져 왔다.
3. **담는 곳 = 적용 범위.** 같은 지시라도 어디에 담느냐가 적용 범위를 정한다 —
   개인 지침(계정 전역) / Projects 지침(프로젝트) / CLAUDE.md(레포) /
   Skills(필요할 때만 동적 로딩). 발표 #6b에서 이 프레임으로 소개.
