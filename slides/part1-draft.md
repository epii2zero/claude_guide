...
*< Claude의 개인 지침(Instructions for Claude)과 ChatGPT 맞춤 설정 >*

- **프롬프트 엔지니어링의 시대** — 지시와 함께 입력해주던 맥락, 규칙
  - **"너는 10년 차 백엔드 엔지니어야"** — 역할(persona)을 부여하기도 함
- **변화: 매 대화마다 채팅에 반복 입력하던 맥락을, 스크린샷처럼 설정에 한 번 통째로 입력** — 모든 대화에 자동 적용
- 여기서 시작된 instructions의 계보가 Projects 지침 → CLAUDE.md → skills까지 이어진다

<!--
#6b | 3분 | 사용자 원안 기반 | 지시의 계보 부 축의 출발 정거장
- 캡처 2장: 왼쪽 Claude 프로필 개인 지침, 오른쪽 ChatGPT 맞춤 설정 (원안 보존본).
- 변화의 서사가 이 장의 핵심: "매번 채팅에 붙여넣던 역할·규칙 →
  설정에 한 번 담아두면 끝. 담는 곳이 생긴 것이 instructions의 시작입니다."
