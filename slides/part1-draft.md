...
<!--
Part 1 슬라이드 초안 (14장, 44분 — 원 번호 #4~15 중 #12 제외, #6→#6a/#6b, #10→#10a/#10b 분할, #13 뒤 #13b 추가, #9는 Memory로 신설)
...
- 한계
  ...
  - **수동 작업** - 모든 맥락을 매번 직접 입력하고, 결과도 직접 받아 옮겨야 함
...
**대화만 가능한 챗봇과의 작업 — 모델의 코드 작성 → 사람의 코드 파일 수정 → 사람의 코드 실행 → 다시 모델에 결과 전달**

AI에게 주어진 도구 (Claude API 기준, 2026.08):

| 구분 | 도구 |
|------|------|
| **서버에서 실행, 결과 반환** | Web search / fetch · Code execution · Tool search |
| **로컬에서 실행 — Claude Code의 시작** | Text editor · Bash · Computer use · Memory |
| **직접 정의 (custom tools)** | 우리 API·DB·사내 서비스, 무엇이든 |

- **사전 학습을 넘은 지식** — 웹 검색으로 최신 정보 습득, 코드 실행으로 얻는 정확한 계산 값·차트
...
- 서사 순서: ① #5의 두 한계(갇힌 지식·수동 작업)를 다시 소환 — 볼드 줄의
  화살표 흐름이 그 모습 → ② 모델이 도구를 "골라 쓰게" 허용한 것이
  Tool use (2024.05 GA) → ③ 도구 사용 루프(요청→선택→실행→관찰)는 말로,
  시각화는 #14 조립 도식에서.
- 표의 "로컬에서 실행" 줄이 Part 2 복선: Text editor + Bash가 곧 Claude Code의
  뼈대 — 원안 v3의 "Claude code의 시작" 표현.
- 개발자용 정확한 한 줄(말로): 실행 주체는 앱/클라이언트 쪽 — 모델은 도구
  "요청"과 결과 "판단"을 맡는다. 그래서 무엇을 열어줄지는 우리가 정한다(#13·#15 복선).
  (서버 도구만 예외적으로 Anthropic 인프라가 실행까지 대행)
...
**지시가 우리가 건네주는 노하우라면, memory는 모델이 자체적으로 쌓아가는 노하우**

- **대화가 끝나도 남는 기억** — 이전 대화의 맥락·선호·진행 상황을 다음 대화가 이어받는다
- **자동 관리** — 대화가 쌓이면 모델이 스스로 요약·갱신 (사용자가 열람·수정·삭제 가능)
- 적용 범위: **계정 전역 memory** / **프로젝트 단위 memory** (다음 장 Project에서)
- API의 **memory tool** — 에이전트가 스스로 파일에 노하우를 기록·재사용 (#8 표의 Memory)

**후배 은유의 전환점: 가르쳐 준 것만 알던 후배가, 스스로 업무 노트를 쓰기 시작한다**

<!--
#9 | 3분 | 신설 (Memory — 모델이 자체적으로 쌓아가는 노하우)
- 배치 이유: 시간순(2025)이 아니라 서사순 — instructions(#6b, 주는 노하우) →
  memory(#9, 쌓이는 노하우) → Project(#10, 팀 공간에 담기)로 "노하우" 축이 이어진다.
  시간순이 아님을 한 마디로 짚고 넘어갈 것.
- Introduction ② 매핑 회수: instructions·skills와 나란한 **memory** —
  "주는 지식"이 아니라 "쌓이는 지식".
- claude.ai memory: 2025년 하반기 도입(Team/Enterprise 먼저, 이후 유료 전반)
  ⚠️ 도입 시점·플랜 조건은 발표 전 재확인.
- 신뢰 포인트(말로): 기억은 투명하게 — 열람·수정·삭제 가능, 시크릿 채팅은
  기억에 남지 않음.
- 검증 습관 연결: 기억도 요약이라 틀릴 수 있다 — 중요한 사실은 확인(#15 복선).
-->
...
**간단한 논문 분석 프로젝트 — 한 번 담아두면, 다음 분석이 이어진다**

1. 첫 채팅에서 **논문을 입력하고 분석**, 실험 결과를 표로 정리
2. 논문 분석 요약과 성능 요약표, 그리고 **분석 요령을 프로젝트 문서로 내보내기**
3. **새로운 채팅**에서 후속 논문과 함께 이전처럼 분석
4. **두 논문의 비교 성능 요약표** 작성 — 이전 채팅의 기준과 요령을 그대로 이어받는다

**같은 패턴이 팀에도**: 팀 컨벤션 가이드 · 온보딩/도메인 지식 · 반복 업무 템플릿

<p class="cite">시연 자료: ISTFTNet — arxiv.org/abs/2203.02395 · Vocos — arxiv.org/abs/2306.00814</p>

<!--
#11 | 6분 (미니 시연 3~4분 포함) | 사용자 원안 v3 "Project Practice" 기반
- [시연 자리] 원안 시나리오 그대로: ISTFTNet 분석·표 정리 → 프로젝트 문서로
  내보내기 → 새 채팅에서 Vocos 분석 + 두 논문 비교표. (part1-demos.md 시연 5를
  이 시나리오로 교체)
- 시연 후: "새 채팅인데 분석 기준이 이어졌죠. 담아둔 문서와 memory 덕분입니다."
- 팀 패턴 3종(컨벤션/온보딩/반복 업무)과 팁(지침은 짧고 명령형, 문서는 선별,
  프로젝트 단위 = 업무 단위, 개인→팀 승격)은 말로 짧게 — 상세는 가이드 문서.
...
**로컬 또는 다른 서버의 프로그램과 통신하기 위한 오픈 표준** — 문제: 연결은 전부 수제였다 (앱 M개 × 도구 N개 = M×N 커스텀 통합)
...
- 다음 장 예고: "그런데 REST API도 이미 있는데 뭐가 다른 거냐 — 좋은 질문이죠."
-->
...
**한 문장: API는 개발자가 읽고 연결하는 규격, MCP는 모델이 읽고 연결하는 규격**

| | 기존 API (REST 등) | MCP |
|---|---------------------|-----|
| **읽는 주체** | 사람 — 개발자가 문서를 읽고 통합 코드 작성 | **모델** — 도구 목록·설명·스키마를 런타임에 발견 |
| **연결 시점** | 개발 시점 — 서비스마다 하드코딩 | 실행 시점 — 서버만 등록하면 동적 연결 |
| **제공 범위** | 엔드포인트 (기능 호출) | tools + resources(참조 데이터) + prompts(작업 템플릿) |
| **규격** | 서비스마다 제각각 (인증·형식·문서) | 하나의 공개 표준 (JSON-RPC 기반) |

- 도구 "설명"이 곧 모델이 읽는 문서 — **좋은 설명 = 좋은 통합**
- 기존 API의 대체가 아니라, **기존 API를 모델이 이해하게 만드는 어댑터**가 MCP 서버

<!--
#13b | 2분 | 개발자 청중의 당연한 질문에 답하는 장
- 핵심 대비: REST 통합은 "사람이 미리 읽고 코드로 굳힌다", MCP는 "모델이
  그때그때 읽고 고른다". 도구 선택의 주체가 모델이라는 #8의 이야기가
  연결 규격까지 내려온 것.
- tools/list로 서버가 자기 도구를 스스로 알려준다(self-describing) —
  클라이언트에 사전 하드코딩이 없다.
- 마지막 불릿이 실전 함의: 사내 시스템에 이미 REST API가 있다면,
  MCP 서버는 그 위에 얇게 얹는 어댑터다. 새로 만드는 게 아니다.
- #8 복선 회수: "좋은 도구 설명 = 좋은 API 문서"가 여기서 실체가 된다.
-->
